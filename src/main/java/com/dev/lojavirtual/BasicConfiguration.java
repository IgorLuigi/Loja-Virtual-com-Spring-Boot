...
//		.encode("admin")).roles("USER", "ADMIN");
		...
		auth.jdbcAuthentication().dataSource(dataSource)
			...
			.authoritiesByUsernameQuery(
					"select funcionario.email as username, papel.nome as authority from permissoes inner join funcionario on funcionario.id = permissoes.funcionario_id inner join papel on permissoes.papel_id where funcionario.email=?")
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
			...
			.exceptionHandling().accessDeniedPage("/negada");
	}
